...
const POST_LOGIN_REDIRECT_STORAGE_KEY = "ryanpdf_post_login_path";
...
const LOGIN_REDIRECT_THROTTLE_KEY = "ryanpdf_last_401_redirect";
...
export async function handleHttpError(error: any): Promise<boolean> {
  ...
  if (status === 401) {
    ...
    if (!isAuthPage && !skipAuthRedirect) {
      ...
      try {
        hadStoredJwt = Boolean(localStorage.getItem("ryanpdf_jwt"));
      } catch {
