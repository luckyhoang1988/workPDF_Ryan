...
export default function ServerLicenseSlide({
  ...
  const overLimitBody = (
    <Trans
      ...
      defaults="Our licensing permits up to <strong>{{freeTierLimit}}</strong> users for free per server. You have <strong>{{overLimitUserCopy}}</strong> RyanPDF users. To continue uninterrupted, upgrade to the RyanPDF Server plan - <strong>unlimited seats</strong>, PDF text editing, and full admin control for $99/server/mo."
    />
  );
  ...
  const freeBody = (
    <Trans
      ...
      defaults="Our <strong>Open-Core</strong> licensing permits up to <strong>{{freeTierLimit}}</strong> users for free per server. To scale uninterrupted, we recommend the RyanPDF Server plan - <strong>unlimited seats</strong> and <strong>SSO support</strong> for $99/server/mo."
    />
  );
