...
/// Attempt to set/prompt for RyanPDF as default PDF handler
#[tauri::command]
...
fn check_default_windows() -> Result<bool, String> {
    ...
    unsafe {
        ...
        let result = (|| -> Result<bool, String> {
            ...
            // Check if it contains "RyanPDF" (case-insensitive)
            // Note: This checks the ProgID registered by the installer
            let is_default = default_str.to_lowercase().contains("ryanpdf");
            Ok(is_default)
        })();
...
fn check_default_macos() -> Result<bool, String> {
    ...
    unsafe {
        ...
        let is_default = handler_str == "com.ryanpdf.app";
        Ok(is_default)
    }
}
...
fn set_default_macos() -> Result<String, String> {
    ...
    unsafe {
        ...
        let pdf_uti = CFString::new("com.adobe.pdf");
        let our_bundle_id = CFString::new("com.ryanpdf.app");
...
fn check_default_linux() -> Result<bool, String> {
    ...
    let is_default = handler.trim() == "ryanpdf.desktop";
    Ok(is_default)
}
...
fn set_default_linux() -> Result<String, String> {
    ...
    let result = Command::new("xdg-mime")
        .args(["default", "ryanpdf.desktop", "application/pdf"])
        .output()
