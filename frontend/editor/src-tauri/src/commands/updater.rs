...
/// a per-machine install writes to `C:\Program Files\RyanPDF`. If the
...
/// `current_exe().parent()` on macOS is `RyanPDF.app/Contents/MacOS/`
...
pub fn can_install_updates() -> CanInstallResult {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let probe = parent.join(".ryanpdf-auto-update-probe.tmp");
